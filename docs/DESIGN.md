...
| KPI cards | Only where they help a decision; currently **Investors** (identification funnel), **Cards** (expiring or expired), **Orders** (buy, sell, pending), **Transactions** (count per operation), **Referral links** (shared, active, joined) and **Bitrix leads** (created, errors, deposit updates). Status or operation values in KPIs are the backend's options, with labels from the schema. The grid sizes itself to the number of cards. Layout (`KpiCard`): **title** (what is counted, muted) on top, **count** below (`text-2xl font-semibold`), and the **icon on the right in a 56 px tinted circle** whose tone matches what's counted (§5 "Color in data"), vertically centered. No hint line under the count. Every number must be computed by the backend: a filtered `count` via `useTableCount`. Anything the backend can't compute yet is `pending`: "—" plus a **Backend pending** badge. Never compute it on the front end. |
...
| **Integration log** | Bitrix status: Created → success · **Error → danger** · Deposit updated → info |
